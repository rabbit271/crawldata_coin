...
def fetch_data():
    ...
    with webdriver.Chrome(service=Service(ChromeDriverManager().install()), options=option) as driver:
        driver.get(f"https://coinmarketcap.com/?nocache={datetime.datetime.now().timestamp()}")
...
headers = ["Name", "Price", "1h %", "24h %", "7d %", "Market Cap", "Volume (24h)", "Timestamp"]
while True:
    data = fetch_data()
    print(tabulate(data, headers=headers, tablefmt="grid"))
    if not os.path.exists('coin_data.csv'):
        pd.DataFrame(data, columns=headers).to_csv('coin_data.csv', mode='w', header=True, index=False)
    else:
        pd.DataFrame(data, columns=headers).to_csv('coin_data.csv', mode='a', header=False, index=False)
    time.sleep(10)
